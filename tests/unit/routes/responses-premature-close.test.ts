...
describe("streamPassthrough premature close handling", () => {
  ...
  it("logs and flags upstream terminal failure frames instead of passing them silently", async () => {
    ...
    expect(metadataCalls.some((metadata) => metadata.terminalFailure === true)).toBe(true);
    expect(recordedCloseEvents).toHaveLength(1);
