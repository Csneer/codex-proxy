...
function rangeBounds(preset: CallRangePreset, timezone: string, now: Date): { from: Date; to: Date } {
  ...
  return { from, to: midnightUtc(timezone, nextParts) };
}
...
export class CallRecordAnalytics {
  ...
  getOverview(input: { range?: CallRangePreset; timezone?: string }): CallOverview {
    const range = resolveCallRange({ ...input, now: this.now() });
    ...
    const buckets = this.store.queryAnalytics<any>(`SELECT substr(completed_at, 1, 13) || ':00:00.000Z' bucket_start, COUNT(*) success FROM call_records WHERE completed_at >= @from AND completed_at < @to GROUP BY bucket_start ORDER BY bucket_start`, { from: range.from, to: range.to });
    return { range: { preset: range.preset, timezone: range.timezone, from: range.from, to: range.to }, generatedAt: this.now().toISOString(), success: { count: Number(current.count ?? 0), previousCount: Number(previous.count ?? 0), lastCompletedAt: current.last_completed_at ?? null }, usage: { inputTokens, outputTokens: Number(current.output_tokens ?? 0), cachedTokens: Number(current.cached_tokens ?? 0), cacheRatio: inputTokens ? Number(current.cached_tokens ?? 0) / inputTokens : 0 }, storage: { permanentBytes: state.databaseBytes, rawBytes: 0, semanticLogicalBytes: 0, rawLogicalBytes: 0, indexBytes: 0, walBytes: 0, totalBytes: state.databaseBytes, measurement: "sqlite-pages-and-logical-sums" }, outcomes: { failure: 0, interrupted: 0, retry: 0 }, series: buckets.map((row) => ({ bucketStart: row.bucket_start, success: Number(row.success) })), models, protocols, contexts, sections: {} };
  }
}
