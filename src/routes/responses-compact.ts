...
import { isRequestableModel, parseModelName, resolveModelId } from "../models/model-store.js";
...
export async function handleCompact(
  ...
  if ((compactRouteMatch?.kind === "api-key" || compactRouteMatch?.kind === "adapter") && supportsCodexAuxiliaryJson(compactRouteMatch.adapter)) {
    const directModel = compactRouteMatch.resolvedModel ?? rawModel;
    return handleCodexAuxiliaryJson({ c, upstream: compactRouteMatch.adapter, path: "responses/compact", body: directModel === rawModel ? body : { ...body, model: directModel }, model: directModel });
  }
