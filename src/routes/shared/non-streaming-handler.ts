...
export async function handleNonStreaming(options: HandleNonStreamingOptions): Promise<Response> {
  ...
  for (let attempt = 1; ; attempt++) {
    try {
      ...
      const metrics = calculateLogMetrics({ startMs: initialStartMs, endMs: Date.now(), model: req.model, usage: result.usage, isStreaming: false });
      c.set("metrics", metrics);
      updateLogEntry(requestId, { status: 200, latencyMs: metrics.durationMs, ttftMs: metrics.ttftMs, durationMs: metrics.durationMs, costUsd: metrics.costUsd, tokensPerSecond: metrics.tokensPerSecond, usage: result.usage, metrics });
      releaseNonStreamingSuccessAccount({
        accountPool,
        entryId: currentEntryId,
        usage: result.usage,
