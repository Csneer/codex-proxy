...
> **Suffixes**: Append `-fast` to any chat model for Fast mode, and use `-high`/`-low`/`-max`/`-ultra` for reasoning effort. E.g. `gpt-5.6-sol-fast`, `gpt-5.6-sol-high-fast`, `gpt-5.6-sol-max`, `gpt-5.6-sol-ultra`. The image model (`gpt-image-2`) does not take suffixes.
>
...
The Ollama Bridge has no separate inbound authentication and injects the service key into `/v1/*` passthrough requests. Its configuration and management API therefore accept loopback hosts only: `localhost`, `127.x.x.x`, or `::1`. Browser CORS is loopback-only as well. For Docker host access, use host networking or a host-local port forward; never publish the bridge to the LAN.
...
To force the main server to localhost-only binding, set `server.host: "127.0.0.1"` in `data/local.yaml` and restart. When using `0.0.0.0`, set strong service and administration keys and restrict the host firewall to trusted networks. The Ollama Bridge remains loopback-only.

### Service and Administration Keys

```yaml
server:
  proxy_api_key: "pwd"    # service key used by inference clients
  # proxy_api_key: null   # no global key; logged-in accounts still have account-level codex-proxy-xxxx keys

dashboard:
  admin_key: "use-the-random-key-generated-on-first-start"  # Dashboard and management API only
```

First startup keeps `server.proxy_api_key: pwd` and separately generates a 64-character hexadecimal `dashboard.admin_key`, persists it to `data/local.yaml`, and prints it once when generated. The credentials are strictly isolated: the service key cannot log in or call management APIs, and the administration key cannot call inference APIs. Localhost, Electron, `127.0.0.1`, and `::1` do not bypass administration login.

Migration is strict: an old `server.proxy_api_key` is never copied or used as an administration fallback. If `dashboard.admin_key` is absent, a new one is generated and persisted. The Dashboard never returns the current administration key. Rotating it revokes every Dashboard session and CSRF token, requiring everyone to sign in again. An authenticated administrator may still change or clear the service key as needed.
...
|----------|-----------|
| `PORT` | `server.port` |
| `CODEX_PROXY_HOST` | `server.host` when `data/local.yaml` does not explicitly set it |
| `CODEX_PROXY_API_KEY` | `server.proxy_api_key` (service/inference key) |
| `CODEX_DASHBOARD_ADMIN_KEY` | `dashboard.admin_key` (administration key; not persisted when supplied by environment) |
